...
void EncodeDistrictKey(int64_t* db_key, uint64_t w_id, uint64_t d_id) {
...
void EncodeCustomerKey(int64_t* db_key, uint64_t w_id, uint64_t d_id, uint64_t c_id) {
    TPCCKey* key = (TPCCKey*) db_key;
...
void EncodeItemKey(int64_t* db_key, uint64_t i_id) {
    TPCCKey* key = (TPCCKey*) db_key;
    key->item.tabid = ITEM;  
    key->item.i_id = i_id;
}

void EncodeStockKey(int64_t* db_key, uint64_t w_id, uint64_t i_id) {
    TPCCKey* key = (TPCCKey*) db_key;
   ...
   for (c_w_id = 1; c_w_id<=warehouse_count; c_w_id++) {
      for (c_d_id = 1; c_d_id<=kDistrictsPerWarehouse; c_d_id++) {
         for (c_id = 1; c_id<=kCustomerPerDistrict; c_id++) {
            ...
            // @formatter:off
            c_csv << db_key << c_balance << csv::endl;
            // @formatter:on
         }
      }
   }
   ...
   for (s_w_id = 1; s_w_id<=warehouse_count; s_w_id++) {
      for (s_i_id = 1; s_i_id<=kItemCount; s_i_id++) {
        EncodeStockKey(&db_key, s_w_id, s_i_id);
         ...
         // @formatter:off
         s_csv << db_key << s_quantity << s_order_cnt << s_ytd 
               << s_remote_cnt << csv::endl;
         // @formatter:on
      }
   }
